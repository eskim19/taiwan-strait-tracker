...
FEEDS = [
    ...
    # 단순 OR 나열만 쓸 것.
    {
        ...
        "lang": "en",
        "kind": "aggregator",
        # 중대(critical): 이 피드가 죽어도 다른 피드가 대신하지 않는다.
        # 로이터·AP·CNN·교도는 자체 RSS 가 없어 이 경로가 유일하고, 실측
        # 코퍼스에서 T1 은 157건 중 7건뿐이다. build.py 는 "전 피드 실패"가
        # 아니면 종료코드 0 을 내므로, 이것들만 죽으면 등급이 조용히 내려간다.
        # 러너의 Azure IP 에서 구글 뉴스가 429/동의 인터스티셜을 줄 수 있는데
        # 가정용 IP 에서는 그게 안 보인다.
        "critical": True,
        "needs_filter": True,
